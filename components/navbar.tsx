...
export default function Navbar() {
  ...
  const toggleDropdown = (title: string, items?: NavItem[]) => {
    const isOpening = openDropdown !== title;
    setOpenDropdown((prev) => (prev === title ? null : title));
    if (isOpening && items) {
      // Auto-select the first item that has children so the right panel is populated immediately
      const firstWithChildren = items.find((i) => i.children && i.children.length > 0);
      setActiveChildDropdown(firstWithChildren ? firstWithChildren.title : null);
    } else {
      setActiveChildDropdown(null);
    }
  };
  ...
  const navigationItems = [
    ...
    },
    {
      ...
      href: "/faculty",
      items: [
        { title: "Full-Time Teaching Faculty", href: "/faculty/fulltimefaculty" },
        { title: "Visiting Faculty", href: "/faculty/visiting" },
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        { title: "Child Health Nursing Lab", href: "/facilities/ChildHealthNursingLab" },
    ...
    },
    {
      ...
      items: [
        ...
        { title: "Extracurricular Activities", href: "/activities/extracurlicular" },
        { title: "Sports", href: "/activities/sports" },
        { title: "Projects", href: "/activities/projects" },
        { title: "Health Camps", href: "/activities/health-camps" },
        { title: "Entertainment", href: "/activities/entertainment" },
      ...
      items: [
        {
          ...
          children: [
            { title: "Annexures Overview", href: "/muhs-mandate/annexures" },
            { title: "Annexure I — Seat Matrix", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/ANNEXURI.pdf`, target: "_blank" },
            { title: "Annexure II — Infrastructure", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/ANNEXURE-II.pdf`, target: "_blank" },
            { title: "Annexure III — Trust Deed", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/AnnexureIII.pdf`, target: "_blank" },
            { title: "Annexure IV — Library Details", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-IV.pdf`, target: "_blank" },
            { title: "Annexure V — Clinical Material", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/ANNEXUREV.pdf`, target: "_blank" },
            { title: "Annexure VI — Total Teaching Staff", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/ANNEXURE-VI.pdf`, target: "_blank" },
            { title: "Annexure VII — Teaching Staff Info", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-VII.pdf`, target: "_blank" },
            { title: "Annexure VIII — Part Time Teachers", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-VIII.pdf`, target: "_blank" },
            { title: "Annexure IX — Non-Teaching Staff", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-IX.pdf`, target: "_blank" },
            { title: "Annexure X — Workshops & Activities", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-X.pdf`, target: "_blank" },
            { title: "Annexure X(2) — Workshops & Activities", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-X(2).pdf`, target: "_blank" },
            { title: "Annexure XI — Activities Info", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-XI.pdf`, target: "_blank" },
            { title: "Annexure XII — AISHE Certificate", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-XII.pdf`, target: "_blank" },
            { title: "Annexure XIII — Question Papers", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-XIII.pdf`, target: "_blank" },
            { title: "Annexure XIII(A) — Subject wise Eligible Examiners List", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/XIIIA.pdf`, target: "_blank" },
            { title: "Annexure XIV — Result", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-XIV.pdf`, target: "_blank" },
            { title: "Annexure XV — Hostel Facility", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-XV.pdf`, target: "_blank" },
            { title: "Annexure XVI — Declaration", href: `${process.env.NEXT_PUBLIC_STATIC_ASSETS}/PDF/muh/Annexure-XVI.pdf`, target: "_blank" },
          ],
        },
  ...
  const firstRowItems = navigationItems.slice(0, 8);
  const secondRowItems = [
    ...navigationItems.slice(8),
    ...singleItems.map((item) => ({ ...item, items: [] })),
  ];

  // ─── Two-panel mega-menu when children exist, plain scrollable list otherwise ───
  const renderDropdownPanel = (items: NavItem[], parentTitle?: string) => {
    const hasChildItems = items.some((item) => item.children);

    if (hasChildItems) {
      // Two-panel layout: left = parent items, right = children of active parent
      const activeItem = items.find((item) => item.title === activeChildDropdown);
      const childrenToShow = activeItem?.children ?? [];

      return (
        <div
          className="absolute top-full left-0 mt-1 z-50 flex shadow-2xl rounded-xl border border-gray-200 bg-white overflow-hidden"
          style={{ minWidth: 520 }}
        >
          {/* Left panel — parent items */}
          <div className="w-56 bg-gray-50 border-r border-gray-100 py-2 flex-shrink-0">
            {items.map((item) => (
              <div key={item.title}>
                {item.children ? (
                  <button
                  ...
                  >
                    <span>{item.title}</span>
                    <ChevronRight className={`w-4 h-4 flex-shrink-0 transition-colors ${activeChildDropdown === item.title ? "text-blue-600" : "text-gray-400"}`} />
                  </button>
                ) : item.target === "_blank" ? (
                  <a
                    href={item.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block px-4 py-2.5 text-sm text-gray-800 hover:bg-blue-50 hover:text-blue-700 transition-colors"
                  >
                    {item.title}
                  </a>
                ) : (
                  <Link
                    href={item.href}
                    className="block px-4 py-2.5 text-sm text-gray-800 hover:bg-blue-50 hover:text-blue-700 transition-colors"
                    onClick={() => { setOpenDropdown(null); setActiveChildDropdown(null); }}
                  >
                    {item.title}
                  </Link>
                )}
          ...
          {/* Right panel — children of active item */}
          <div className="flex-1 py-2 min-w-0" style={{ minWidth: 256 }}>
    ...
    // Plain list — max-height with scroll so long lists (Facilities 17 items) show fully
    return (
      <div
        className="absolute top-full left-0 mt-1 z-50 bg-white rounded-xl shadow-2xl border border-gray-200 py-2 overflow-y-auto overflow-x-hidden"
        style={{ minWidth: 240, maxHeight: "calc(100vh - 120px)" }}
      >
        {items.map((item) =>
          item.target === "_blank" ? (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="block px-4 py-2.5 text-sm text-gray-800 hover:bg-blue-50 hover:text-blue-700 transition-colors whitespace-nowrap"
            >
              {item.title}
            </a>
          ) : (
  ...
  const renderNavItem = (item: {
    ...
    <div key={item.title} className="relative">
      {item.items.length > 0 ? (
        <>
          <button
            onClick={() => toggleDropdown(item.title, item.items as NavItem[])}
            className={`inline-flex items-center gap-1 px-3 py-1.5 rounded text-sm font-medium transition-all duration-200 ${
              openDropdown === item.title
                ? "bg-white/20 text-white"
                : "text-white/90 hover:text-white hover:bg-white/10"
          ...
          {openDropdown === item.title && renderDropdownPanel(item.items, item.title)}
        </>
      ) : item.target === "_blank" ? (
        <a
          href={item.href}
    ...
    <header className="sticky top-0 z-50 w-full shadow-xl" ref={navbarRef}>
      ...
      <div className="bg-white text-foreground py-2 px-4 overflow-hidden">
        <div className="flex items-center justify-center">
          <div className="whitespace-nowrap animate-marquee">
            <span className="font-medium text-sm">
              🎓 Admissions Open for 2025-26 Academic Year | Limited Seats Available | Apply Now!
            </span>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center">
            ...
            <div className="flex lg:hidden ml-auto">
              <Sheet open={isOpen} onOpenChange={setIsOpen}>
                ...
                <SheetContent side="left" className="w-[85%] sm:w-[350px] bg-white text-gray-800 px-4 pt-6">
                  <SheetHeader className="mb-4">
                      ...
                      <div key={item.title} className="border-b border-gray-100 pb-2">
                        {item.items && item.items.length > 0 ? (
                          <Collapsible>
                            ...
                            <CollapsibleContent className="pl-4 pt-1 space-y-1">
                              {(item.items as NavItem[]).map((subItem) =>
                                subItem.children ? (
                                  <Collapsible key={subItem.title}>
                                    <CollapsibleTrigger className="flex justify-between items-center w-full py-1.5 text-sm hover:text-blue-700">
                                      <span>{subItem.title}</span>
